...
async def compress_video(client, message: Message, start_msg):
    try:
        ...
        if original_size == 0:
            logger.error("El archivo descargado está vacío (0 bytes)")
            await msg.edit("❌ **Error: El video descargado está vacío**")
        ...
        try:
            ...
            # Verificar si el archivo comprimido existe y tiene tamaño
            if not os.path.exists(compressed_video_path):
                logger.error(f"El archivo comprimido no existe: {compressed_video_path}")
                await msg.edit("❌ **Error: La compresión falló, no se generó archivo de salida**")
                if original_video_path and os.path.exists(original_video_path):
                    os.remove(original_video_path)
                await remove_active_compression(user_id)
                unregister_cancelable_task(user_id)
            ...
            if compressed_size == 0:
                logger.error("El archivo comprimido está vacío (0 bytes)")
                await msg.edit("❌ **Error: El video comprimido está vacío**")
                if original_video_path and os.path.exists(original_video_path):
                    os.remove(original_video_path)
                if compressed_video_path and os.path.exists(compressed_video_path):
                    os.remove(compressed_video_path)
            ...
            try:
                ...
                if not os.path.exists(compressed_video_path):
                    ...
                    await upload_msg.edit("❌ **Error: El archivo comprimido no existe**")
                    if original_video_path and os.path.exists(original_video_path):
                        os.remove(original_video_path)
                    await remove_active_compression(user_id)
                    unregister_cancelable_task(user_id)
